...
from info import CAPTION_LANGUAGES, DATABASE_URI, DATABASE_URI2, DATABASE_NAME, COLLECTION_NAME, USE_CAPTION_FILTER, MAX_B_TN, Lootere_MOVIE_UPDATE_CHANNEL, OWNERID
...
async def send_msg(bot, filename, caption): 
    try:
        ...
        if await add_name(OWNERID, filename):
            ...
            if resized_poster:
                await bot.send_photo(chat_id=Lootere_MOVIE_UPDATE_CHANNEL, photo=resized_poster, caption=text, reply_markup=InlineKeyboardMarkup(btn))
            else:              
                await bot.send_message(chat_id=Lootere_MOVIE_UPDATE_CHANNEL, text=text, reply_markup=InlineKeyboardMarkup(btn))
